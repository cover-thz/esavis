...
class CoverProc:
    ...
    #  Shared by all data sources (THz .dat, DAQ, external HDF5)      #
    ###################################################################
    ...
    def postproc_data(s, rangelines_array, az_array, el_array, 
        ...
        if process_frame:
            #############################################################
            # Stage A: radar-specific spectral / range conversion
            #############################################################
            ...
            #############################################################
            # Build PowerCube and delegate to Stage B
            #############################################################
            # x/y coords in encoder units converted to metres for the
            # PowerCube interface (existing THz path)
            az_to_cm = cfg_dict.get("az_encoder_to_cm", 0.032)
            el_to_cm = cfg_dict.get("el_encoder_to_cm", 0.032)
            x_coords_m = s.coarse_az_1d * az_to_cm / 100.0
            y_coords_m = s.coarse_el_1d * el_to_cm / 100.0

            power_cube = make_power_cube(coarse_power_grid,
                                         s.r_grid_valids,
                                         range_lut_cm,
                                         x_coords_m, y_coords_m)

            (frame_out, aux_data_out,
             frame_id_out) = s.process_power_cube(power_cube, cfg_dict,
                                                  update_id, dbg_prof)
            ...
            if dbg_prof:
                ...
                dur_ms = (postamble_end - misc_dur_1_end)/1e6
